...
class Object_Tracker():
    ...
    def __init__(self):
        ...
        # initialize tracker
        self.tracker = Tracker(metric, max_age=500)
    ...
    def image_track(self, image):
        ...
        movenet_frame = tf.image.resize_with_pad(tf.expand_dims(frame, axis=0), 384, 640)
        movenet_frame = tf.cast(movenet_frame, dtype=tf.int32)
        movenet_results = self.movenet(movenet_frame)
        ...
        reinit_bboxes = loop_through_people(frame, keypoints_with_scores, 0.30, bounding_boxes)
        ...
        indices = []
        if len(boxs) > 0:
            indices = tf.image.non_max_suppression(boxs, scores, 50, iou_threshold=0.45,
                score_threshold=0.5)
        ...
        if reinit_bboxes is not None:
            ...
            max_iou_track = get_max_iou_track(reinit_bboxes, self.tracker.tracks)
            self.saved_id = max_iou_track.track_id
        ...
        for track in self.tracker.tracks:
            ...
            if not track.track_id == self.saved_id:
                continue
            bbox = track.to_tlbr()
            output = [[(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2, bbox[2] - bbox[0], bbox[3] - bbox[1]], [1]]
        ...
        return output[0], output[1]  #CenterX CenterY, Width Height
    ...
    try:
        ...
        # for image in sorted(os.scandir("./data/video/images"), key=lambda x: x.path): 
            ...
            # with Image.open(image.path) as im:
                # print(ob_tracker.image_track(im))
